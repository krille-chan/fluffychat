...
class HomeserverPickerController extends State<HomeserverPicker> {
  ...
  Future<void> checkHomeserverAction({bool legacyPasswordLogin = false}) async {
    ...
    try {
      ...
      final client = await Matrix.of(context).getLoginClient();
      final (_, _, loginFlows, authMetadata) =
          await client.checkHomeserver(homeserver, fetchAuthMetadata: true);
  ...
  void oidcLoginAction() async {
    final redirectUrl = kIsWeb
        ? Uri.parse(html.window.location.href).resolveUri(
    ...
    final urlScheme =
        isDefaultPlatform ? redirectUrl.scheme : "http://localhost:3001";
