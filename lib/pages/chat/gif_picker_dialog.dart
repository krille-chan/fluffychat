...
class GifPickerDialogState extends State<GifPickerDialog> {
  ...
  Future<void> _loadFeaturedGifs() async {
    ...
    try {
      ...
      final response = await TenorApi.getFeaturedGifs(client: client);
      setState(() {
        gifs = response.results;
        isLoading = false;
        _nextPos = response.results.length + _nextPos;
        _hasMoreResults = response.results.length != 0;
      });
    } catch (e) {
      setState(() {
        isLoading = false;
  ...
  Future<void> _searchGifs(String query) async {
    ...
    try {
      final client = Matrix.of(context).client;
      final response = await TenorApi.searchGifs(query, client: client);
      setState(() {
        gifs = response.results;
        isLoading = false;
        _nextPos = response.results.length + _nextPos;
